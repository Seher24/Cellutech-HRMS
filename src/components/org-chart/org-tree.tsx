...
function OrgNodeView({ node, depth }: { node: OrgNode; depth: number }) {
  return (
    <div className={cn(depth > 0 && "ml-4 border-l border-slate-200 pl-4 md:ml-8")}>
      <div className="mb-3 inline-block min-w-[220px] rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="text-sm font-semibold text-slate-900">{node.name}</div>
        <div className="text-xs text-teal-700">{node.title}</div>
        <div className="mt-1 text-[11px] text-slate-500">
          {node.department ?? node.role.replaceAll("_", " ")}
        </div>
        <div className="text-[11px] text-slate-400">{node.email}</div>
      </div>
